...
| 22 | « Que l'infobulle précise aussi de quel jeu de données vient la valeur » | **Provenance sur les 28 cibles interactives.** Pied d'infobulle nommant le ou les classeurs : comptes nationaux pour la VA, crédit bancaire pour l'encours, FINAN pour les agences — et **les deux** pour l'IFR, l'écart et la réallocation, qui sont des rapports entre eux. Aucun nom de fichier n'est écrit dans le code d'affichage : les libellés viennent de `DATA.meta.sources`, déjà produit par le pipeline, donc ils suivent si une source change. 672 infobulles vérifiées sur 24 scénarios × 2 thèmes. |
